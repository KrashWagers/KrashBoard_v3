...
import { ResponsiveContainer, ComposedChart, BarChart, LineChart, Bar, Line, XAxis, YAxis, CartesianGrid, Tooltip, Cell, Legend, LabelList, Label, ReferenceLine } from 'recharts'
    ...
    <div className="bg-[#1a1a1a] border-2 border-gray-700 rounded-lg p-4 shadow-2xl">
      ...
      <div className="space-y-2">
        <div className="flex justify-between items-center gap-6">
          ...
          <span className="text-white font-bold text-lg">{data.goals}</span>
        </div>
        
        {data.shots_on_goal !== undefined && (
          <div className="flex justify-between items-center gap-6">
            <span className="text-gray-400 text-xs">Shots on Goal</span>
            <span className="text-gray-300 text-sm">{data.shots_on_goal}</span>
          </div>
        )}
        ...
        {data.pp_goals !== undefined && data['5v5_goals'] !== undefined && (
          <>
            ...
            <div className="flex justify-between items-center gap-6">
              <span className="text-gray-400 text-xs">PP Goals</span>
              <span className="text-blue-400 text-sm">{data.pp_goals}</span>
            </div>
            <div className="flex justify-between items-center gap-6">
              <span className="text-gray-400 text-xs">5v5 Goals</span>
...
export function GoalsDashboard({ gamelogs, selectedProp, lineValue }: GoalsDashboardProps) {
  const [showsRollingAvg, setShowsRollingAvg] = useState({ shotsOnGoal: false, sogHD: false })
  ...
  const chartData = useMemo(() => {
    return gamelogs.map((game) => {
      ...
      sog_HD: game.sog_HD,
      shifts: game.shifts,
      pp_goals: game.pp_goals,
      '5v5_goals': game['5v5_goals'],
      goals_total: (game.pp_goals || 0) + (game['5v5_goals'] || 0),
      sat_HD: game.sat_HD,
      sat_MD: game.sat_MD,
      sat_LD: game.sat_LD,
  ...
  // Calculate rolling averages for each chart independently
  ...
  const sogHDData = useMemo(() => {
    return chartData.map((_, i) => {
      const start = Math.max(0, i - 3 + 1)
      const subset = chartData.slice(start, i + 1)
      const avg = subset.reduce((sum, d) => sum + d.sog_HD, 0) / subset.length
      return { ...chartData[i], sog_HD_avg: avg }
    })
  }, [chartData])

  // Determine color for goals - Simple green/red based on hit/miss
  const getGoalBarColor = (goals: number) => {
    const isOver = selectedProp?.ou === 'Over'
    const line = lineValue
    
    // Hit = Green, Miss = Red
    if (isOver) {
      return goals > line ? '#22c55e' : '#ef4444'
    } else {
      return goals < line ? '#22c55e' : '#ef4444'
    }
  }
  ...
  return (
    <div className="space-y-4">
              ...
              <ComposedChart data={recentData} margin={{ top: 10, right: 10, left: 0, bottom: 5 }}>
                ...
                >
                  ...
                  {recentData.map((entry, index) => (
                    <Cell key={`cell-${index}`} fill={getGoalBarColor(entry.goals)} />
                  ))}
                </Bar>
                {showShifts && (
                  <Line
      ...
      <div className="grid grid-cols-2 gap-4">
        {/* Shots on Goal with Toggle */}
        <Card className="border border-gray-700 bg-[#171717] transition-all duration-200 hover:border-gray-600">
          <CardHeader className="p-3 pb-2">
            <div className="flex items-center justify-between">
              <CardTitle className="text-sm">Shots on Goal</CardTitle>
              <label className="flex items-center gap-2 text-xs text-muted-foreground cursor-pointer">
                <input 
                  type="checkbox" 
                  checked={showsRollingAvg.shotsOnGoal} 
                  onChange={() => setShowsRollingAvg(prev => ({ ...prev, shotsOnGoal: !prev.shotsOnGoal }))} 
          ...
          <CardContent className="p-0 pb-0 pr-2">
            <ResponsiveContainer width="100%" height={260}>
              {showsRollingAvg.shotsOnGoal ? (
                <LineChart data={shotsOnGoalData} margin={{ top: 10, right: 10, left: 0, bottom: 5 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#374151" vertical={false} />
                  <XAxis dataKey="opponentAndDate" tick={false} height={0} />
                  <YAxis tick={{ fontSize: 10, fill: '#9ca3af' }} />
                  <Tooltip content={<CustomTooltip />} />
                  <Line type="monotone" dataKey="shots_on_goal_avg" stroke="#5F85DB" strokeWidth={2} dot={false} />
                </LineChart>
              ) : (
                <BarChart data={recentData} margin={{ top: 10, right: 10, left: 0, bottom: 5 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#374151" vertical={false} />
                  <XAxis dataKey="opponent" tick={false} height={0} />
                  <YAxis tick={{ fontSize: 10, fill: '#9ca3af' }} />
                  <Tooltip content={<CustomTooltip />} />
        ...
        {/* High Danger Shots with Toggle */}
        <Card className="border border-gray-700 bg-[#171717] transition-all duration-200 hover:border-gray-600">
          <CardHeader className="p-3 pb-2">
            <div className="flex items-center justify-between">
              <CardTitle className="text-sm">High Danger Shots</CardTitle>
              <label className="flex items-center gap-2 text-xs text-muted-foreground cursor-pointer">
                <input 
                  type="checkbox" 
                  checked={showsRollingAvg.sogHD} 
                  onChange={() => setShowsRollingAvg(prev => ({ ...prev, sogHD: !prev.sogHD }))} 
                  className="w-3 h-3 cursor-pointer" 
                />
                Rolling Avg
              </label>
          ...
          <CardContent className="p-0 pb-0 pr-2">
            <ResponsiveContainer width="100%" height={260}>
              {showsRollingAvg.sogHD ? (
                <LineChart data={sogHDData} margin={{ top: 10, right: 10, left: 0, bottom: 5 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#374151" vertical={false} />
                  <XAxis dataKey="opponentAndDate" tick={false} height={0} />
                  <YAxis tick={{ fontSize: 10, fill: '#9ca3af' }} />
                  <Tooltip content={<CustomTooltip />} />
                  <Line type="monotone" dataKey="sog_HD_avg" stroke="#5F85DB" strokeWidth={2} dot={false} />
                </LineChart>
              ) : (
                <BarChart data={recentData} margin={{ top: 10, right: 10, left: 0, bottom: 5 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#374151" vertical={false} />
        ...
        <Card className="border border-gray-700 bg-[#171717] transition-all duration-200 hover:border-gray-600">
          <CardHeader className="p-3 pb-2">
            <CardTitle className="text-sm font-semibold">Goals by Situation</CardTitle>
          </CardHeader>
          <CardContent className="p-0 pb-0 pr-2">
            <ResponsiveContainer width="100%" height={260}>
              <BarChart data={recentData} margin={{ top: 10, right: 10, left: 0, bottom: 5 }}>
                ...
                <Bar dataKey="pp_goals" stackId="a" name="Power Play Goals" fill="#5F85DB" radius={[4, 4, 0, 0]} />
                <Bar dataKey="5v5_goals" stackId="a" name="5v5 Goals" fill="#4a90e2" radius={[0, 0, 0, 0]}>
                  <LabelList dataKey="goals_total" position="top" style={{ fill: '#f3f4f6', fontSize: '9px' }} formatter={(value: any) => value > 0 ? value : ''} />
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </CardContent>
