...
interface MainPointsChartProps {
  chartData: ChartData[]
...
const getPointsBarColor = (points: number | undefined, selectedProp: { ou: string }, lineValue: number) => {
...
export function MainPointsChart({
  chartData,
  selectedProp,
  lineValue,
  chartSettings,
      ...
      <CardContent className="p-0 pb-0 pr-2 transition-opacity duration-300 flex-1 min-h-[240px]">
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart
            ...
            key={`main-points-chart-${recentData.length}`}
          >
            ...
            <YAxis
              ...
              domain={getYAxisDomain("points", recentData)}
              width={35}
              ticks={getYAxisTicks(Math.max(...recentData.map((d) => d.points || 0)), "points")}
            />
            ...
            <Bar dataKey="points" yAxisId="left" radius={getBarRadius(recentData.length)} isAnimationActive={false}>
              <LabelList
                dataKey="points"
              ...
              {recentData.map((entry, index) => (
                <Cell key={`cell-${index}`} fill={getPointsBarColor(entry.points, selectedProp, lineValue)} />
              ))}
            </Bar>
          </ComposedChart>
        </ResponsiveContainer>
