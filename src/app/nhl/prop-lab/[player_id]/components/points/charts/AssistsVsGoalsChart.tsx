...
interface AssistsVsGoalsChartProps {
  chartData: ChartData[]
}

export function AssistsVsGoalsChart({ chartData }: AssistsVsGoalsChartProps) {
  return (
    <Card variant="secondary" className="transition-opacity duration-300 flex flex-col">
      ...
      <CardContent className="p-0 pb-0 pr-2 transition-opacity duration-300 flex-1 min-h-[200px]">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={chartData} margin={{ top: 5, right: 5, left: 5, bottom: 2 }} key={`assists-goals-${chartData.length}`}>
            ...
            <Bar dataKey="assists" stackId="a" name="Assists" fill="#22c55e" radius={[0, 0, 0, 0]} isAnimationActive={false} />
            <Bar dataKey="goals" stackId="a" name="Goals" fill="#ef4444" radius={getBarRadius(chartData.length)} isAnimationActive={false}>
              <LabelList
                dataKey="assists_goals_total"
                position="top"
