...
interface SOGBySituationChartProps {
  chartData: ChartData[]
}

export function SOGBySituationChart({ chartData }: SOGBySituationChartProps) {
  return (
    <Card variant="secondary" className="transition-opacity duration-300 flex flex-col">
      ...
      <CardContent className="p-0 pb-0 pr-2 transition-opacity duration-300 flex-1 min-h-[200px]">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={chartData} margin={{ top: 5, right: 5, left: 5, bottom: 2 }} key={`sog-situation-${chartData.length}`}>
            ...
            <YAxis
              ...
              width={35}
              domain={getYAxisDomain("shots_on_goal", chartData)}
              ticks={getYAxisTicks(Math.max(...chartData.map((d) => d.shots_on_goal ?? 0)), "shots_on_goal")}
            />
            <RechartsTooltip content={<CustomTooltip />} />
            <Legend wrapperStyle={{ fontSize: "11px", paddingTop: "5px" }} iconSize={10} />
            <Bar dataKey="pp_shots_on_goal" stackId="a" name="PP SOG" fill="#38bdf8" radius={[0, 0, 0, 0]} isAnimationActive={false} />
            <Bar dataKey="5v5_shots_on_goal" stackId="a" name="5v5 SOG" fill="#1d4ed8" radius={getBarRadius(chartData.length)} isAnimationActive={false}>
              <LabelList
                dataKey="sog_total"
                position="top"
