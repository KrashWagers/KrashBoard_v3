...
import { MainPPPointsChart } from "./charts/MainPPPointsChart"
import { PPTOIChart } from "./charts/PPTOIChart"
import { PPPointsBySituationChart } from "./charts/PPPointsBySituationChart"
import { PPPointsShareChart } from "./charts/PPPointsShareChart"
import { TeamPPGoalsChart } from "./charts/TeamPPGoalsChart"
...
export function PropLabTab({
  ...
  const recentData = useMemo(() => {
    ...
    if (chartSettings.showRollingAverage && ppPointsRollingAverageData.length > 0) {
      data = data.map((item, i) => ({
        ...item,
        rollingAvg: ppPointsRollingAverageData[i]?.rollingAvg,
      }))
    }

    if (chartSettings.showMovingAverage && ppPointsMovingAverageData.length > 0) {
      data = data.map((item, i) => ({
        ...item,
        movingAvg: ppPointsMovingAverageData[i]?.movingAvg,
      }))
    }

    if (chartSettings.showTrendLine && ppPointsTrendLineData.length > 0) {
      data = data.map((item, i) => ({
        ...item,
        trendValue: ppPointsTrendLineData[i]?.trendValue,
      }))
    }
    ...
    chartSettings,
    averageLineValue,
    ppPointsRollingAverageData,
    ppPointsMovingAverageData,
    ppPointsTrendLineData,
  ])
  ...
  return (
    <div className="space-y-2">
      <div className="grid grid-cols-3 gap-3">
        ...
        />
        <PPTOIChart chartData={chartData} />
      </div>

      <div className="grid grid-cols-3 gap-3">
        <PPPointsBySituationChart chartData={chartData} />
        <PPPointsShareChart chartData={chartData} />
        <TeamPPGoalsChart chartData={chartData} />
      </div>
