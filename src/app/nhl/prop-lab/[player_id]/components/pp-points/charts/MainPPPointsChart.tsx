...
interface MainPPPointsChartProps {
  chartData: ChartData[]
...
const getPPPointsBarColor = (points: number | undefined, selectedProp: { ou: string }, lineValue: number) => {
  const pointsValue = points ?? 0
  const isOver = selectedProp?.ou === "Over"
  if (isOver) return pointsValue > lineValue ? "#16A34A" : "#ef4444"
  return pointsValue < lineValue ? "#16A34A" : "#ef4444"
}

export function MainPPPointsChart({
  chartData,
  selectedProp,
  lineValue,
  chartSettings,
      ...
      <CardContent className="p-0 pb-0 pr-2 transition-opacity duration-300 flex-1 min-h-[240px]">
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart
            ...
            key={`main-pp-points-chart-${recentData.length}`}
          >
            ...
            <YAxis
              ...
              domain={getYAxisDomain("pp_points", recentData)}
              width={35}
              ticks={getYAxisTicks(Math.max(...recentData.map((d) => d.pp_points || 0)), "pp_points")}
            />
            ...
            <Bar dataKey="pp_points" yAxisId="left" radius={getBarRadius(recentData.length)} isAnimationActive={false}>
              <LabelList
                dataKey="pp_points"
              ...
              {recentData.map((entry, index) => (
                <Cell key={`cell-${index}`} fill={getPPPointsBarColor(entry.pp_points, selectedProp, lineValue)} />
              ))}
            </Bar>
          </ComposedChart>
        </ResponsiveContainer>
