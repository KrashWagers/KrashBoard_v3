...
interface ShootingPercentageChartProps {
  chartData: ChartData[]
  shootingPctData: Array<ChartData & { shooting_pct_rolling_avg?: number }>
  seasonAvg: number
}

export function ShootingPercentageChart({ chartData, shootingPctData, seasonAvg }: ShootingPercentageChartProps) {
  // Calculate max value for Y-axis domain (filter out undefined values)
  const validValues = shootingPctData
    .map(d => d.shooting_pct_rolling_avg)
  ...
  const currentAvg = shootingPctData.length > 0 
    ? (shootingPctData.slice().reverse().find(d => d.shooting_pct_rolling_avg !== undefined)?.shooting_pct_rolling_avg ?? 0)
    : 0
    ...
    <Card variant="secondary" className="transition-opacity duration-300 flex flex-col">
      ...
      <CardContent className="p-0 pb-0 pr-2 transition-opacity duration-300 flex-1 min-h-[200px]">
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart 
            data={shootingPctData} 
            margin={{ top: 5, right: 5, left: 5, bottom: 2 }}
            style={{ transition: 'opacity 0.3s ease-in-out' }}
            key={`shooting-pct-chart-${shootingPctData.length}`}
          >
            ...
            <Area
              type="monotone"
              dataKey="shooting_pct_rolling_avg"
              fill="#16A34A"
              fillOpacity={0.2}
              stroke="none"
