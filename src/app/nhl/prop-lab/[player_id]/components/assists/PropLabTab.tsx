...
import { MainAssistsChart } from "./charts/MainAssistsChart"
import { AssistBreakdownChart } from "./charts/AssistBreakdownChart"
import { AssistsBySituationChart } from "./charts/AssistsBySituationChart"
import { AssistsVsTeamAssistsChart } from "./charts/AssistsVsTeamAssistsChart"
import { AssistsVsPointsChart } from "./charts/AssistsVsPointsChart"
...
export function PropLabTab({
  ...
  const recentData = useMemo(() => {
    ...
    if (chartSettings.showRollingAverage && assistsRollingAverageData.length > 0) {
      data = data.map((item, i) => ({
        ...item,
        rollingAvg: assistsRollingAverageData[i]?.rollingAvg,
      }))
    }

    if (chartSettings.showMovingAverage && assistsMovingAverageData.length > 0) {
      data = data.map((item, i) => ({
        ...item,
        movingAvg: assistsMovingAverageData[i]?.movingAvg,
      }))
    }

    if (chartSettings.showTrendLine && assistsTrendLineData.length > 0) {
      data = data.map((item, i) => ({
        ...item,
        trendValue: assistsTrendLineData[i]?.trendValue,
      }))
    }
    ...
    chartSettings,
    averageLineValue,
    assistsRollingAverageData,
    assistsMovingAverageData,
    assistsTrendLineData,
  ])
  ...
  return (
    <div className="space-y-2">
      <div className="grid grid-cols-3 gap-3">
        ...
        />
        <AssistBreakdownChart chartData={chartData} />
      </div>

      <div className="grid grid-cols-3 gap-3">
        <AssistsBySituationChart chartData={chartData} />
        <AssistsVsTeamAssistsChart chartData={chartData} />
        <AssistsVsPointsChart chartData={chartData} />
      </div>
