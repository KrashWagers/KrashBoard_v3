...
interface AssistsVsPointsChartProps {
  chartData: ChartData[]
}

export function AssistsVsPointsChart({ chartData }: AssistsVsPointsChartProps) {
  return (
    <Card variant="secondary" className="transition-opacity duration-300 flex flex-col">
      ...
      <CardContent className="p-0 pb-0 pr-2 transition-opacity duration-300 flex-1 min-h-[200px]">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={chartData} margin={{ top: 5, right: 5, left: 5, bottom: 2 }} key={`assists-points-${chartData.length}`}>
            ...
            <Bar dataKey="points" stackId="a" name="Points" fill="#38bdf8" radius={getBarRadius(chartData.length)} isAnimationActive={false}>
              <LabelList
                dataKey="assists_points_total"
                position="top"
