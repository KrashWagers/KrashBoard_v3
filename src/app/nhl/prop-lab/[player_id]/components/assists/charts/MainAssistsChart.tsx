...
interface MainAssistsChartProps {
  chartData: ChartData[]
...
const getAssistBarColor = (assists: number | undefined, selectedProp: { ou: string }, lineValue: number) => {
  const assistsValue = assists ?? 0
  const isOver = selectedProp?.ou === "Over"
  if (isOver) return assistsValue > lineValue ? "#16A34A" : "#ef4444"
  return assistsValue < lineValue ? "#16A34A" : "#ef4444"
}

export function MainAssistsChart({
  chartData,
  selectedProp,
  lineValue,
  chartSettings,
      ...
      <CardContent className="p-0 pb-0 pr-2 transition-opacity duration-300 flex-1 min-h-[240px]">
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart
            ...
            key={`main-assists-chart-${recentData.length}`}
          >
            ...
            <YAxis
              ...
              domain={getYAxisDomain("assists", recentData)}
              width={35}
              ticks={getYAxisTicks(Math.max(...recentData.map((d) => d.assists || 0)), "assists")}
            />
            ...
            <Bar dataKey="assists" yAxisId="left" radius={getBarRadius(recentData.length)} isAnimationActive={false}>
              <LabelList
                dataKey="assists"
              ...
              {recentData.map((entry, index) => (
                <Cell key={`cell-${index}`} fill={getAssistBarColor(entry.assists, selectedProp, lineValue)} />
              ))}
            </Bar>
          </ComposedChart>
        </ResponsiveContainer>
