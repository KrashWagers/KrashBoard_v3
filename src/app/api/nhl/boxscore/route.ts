...
async function fetchBoxscore(gameId: string): Promise<BoxscoreResponse> {
  const upstream = `https://api-web.nhle.com/v1/gamecenter/${gameId}/boxscore`
  const response = await fetch(upstream, { cache: 'no-store' })
  if (!response.ok) {
    throw new Error(`Upstream boxscore fetch failed: ${response.status}`)
  }
...
export async function GET(request: NextRequest) {
  ...
  try {
    const cached = getCached<BoxscoreResponse>(cacheKey)
    ...
    logger.error('Failed to fetch NHL boxscore', error)
    return NextResponse.json({ error: 'Failed to fetch NHL boxscore' }, { status: 500 })
  }
}
